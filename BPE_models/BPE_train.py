...
def bpe_train(config):
    ...
    if not os.path.exists(ROOT_PATH / "BPE_models" / full_name):
        ...
        shutil.move(ROOT_PATH / full_name, ROOT_PATH / "BPE_models")
        os.remove(ROOT_PATH / vocab_name)
...
def kenlm_path():
    lm_gzip_path = ROOT_PATH /'BPE_models / 4_gram.arpa.gz'
    ...
    uppercase_lm_path = ROOT_PATH /'BPE_models / 4_gram_upper.arpa.gz'
